...
CHARACTER_SIZE = 64
JOYSTICK_RADIUS = 50
MOVEMENT_DEADZONE = 20  # Radius around the anchor within which no movement occurs
WHITE = (255, 255, 255)
LIGHT_BLUE = (173, 216, 230)
TRANSPARENT_GRAY = (200, 200, 200, 150)  # RGBA for semi-transparent experience bar
LARGE_AREA_RADIUS = 5000  # Maximum range to remember orbs
LOADED_AREA_RADIUS = 3000  # Maximum range to spawn orbs around the player

# Orb settings
ORB_SIZE = 8  # Diameter of orbs
ORB_BATCH_SIZE = 50  # Number of orbs to spawn per batch
MAX_ORBS = 500  # Maximum number of orbs allowed at a time
EXPERIENCE_BAR_HEIGHT = 15

# Enemy settings
ENEMY_SIZE = 20  # Square enemy dimensions
ENEMY_SPEED = 2  # Speed of enemies
ENEMY_DAMAGE = 10  # Damage dealt by enemies
MAX_ENEMIES = 100  # Maximum number of enemies allowed at a time
SPAWN_RADIUS = 600  # Radius around the player for enemy spawning
ENEMY_COLLISION_RADIUS = 25 

# Base stats
BASE_PICKUP_RANGE = 100  # Pixels
BASE_SPEED = 5  # Movement units per frame
...
class Character:
    def __init__(self):
        ...
        self.regeneration = 0.01  # Percentage of max health per second
    ...
    def handle_collisions(self, enemies):
        ...
        for other in enemies:
            ...
            if distance < ENEMY_COLLISION_RADIUS and distance > 0:
                dx = self.x - other.x
                dy = self.y - other.y
                self.x += dx / distance * 0.5  # Push this enemy slightly away
...
def main():
    ...
    while running:
        ...
        for orb in world.orbs[:]:
            if orb.is_near(world.offset_x, world.offset_y, character.get_pickup_range()):
        ...
        for enemy in enemies:
            ...
            if enemy.is_colliding(world.offset_x, world.offset_y):
